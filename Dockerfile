...
RUN git clone https://github.com/runpod-workers/worker-comfyui.git /tmp/worker && \
    cp /tmp/worker/src/handler.py / && \
    cp /tmp/worker/src/start.sh / && \
    (pip install -r /tmp/worker/requirements.txt --quiet || true) && \
    chmod +x /start.sh && \
    rm -rf /tmp/worker
